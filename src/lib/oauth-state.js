...
function decodeState(state) {
  ...
  if (!encodedPayload || !signature) throw new Error("OAuth-State fehlt oder ist ungueltig.");
  ...
  if (
    ...
    throw new Error("OAuth-State Signatur ist ungueltig.");
  }
...
export async function verifyOAuthState(state) {
  ...
  if (!csrf || csrf !== payload.csrf) {
    throw new Error("OAuth-CSRF-Pruefung fehlgeschlagen.");
  }
