...
export async function POST(request) {
  ...
  try {
    ...
    if (!isSmtpConfigured(mitarbeiter)) {
      return NextResponse.json(
        { error: `SMTP für ${mitarbeiter.name} ist noch nicht konfiguriert.` },
        { status: 400 }
      );
    }
    ...
    if (!isStillFree) {
      return NextResponse.json(
        { error: "Dieser Slot ist leider nicht mehr verfügbar." },
        { status: 409 }
      );
    }
    ...
    let callerId = null;
    if (callerSlug) {
      const caller = await prisma.caller.findUnique({ where: { slug: callerSlug } });
      if (caller?.isActive) callerId = caller.id;
    }
    ...
    await prisma.meetingReminder.createMany({
      data: reminderScheduleData({
        ...
        startTime,
        leadOptions: mitarbeiter.reminderConfigs ?? mitarbeiter.reminderLeadOptions,
      }),
      skipDuplicates: true,
    });
