...
import { DEFAULT_EMAIL_COLORS, DEFAULT_EMAIL_TEMPLATES, isSmtpConfigured } from "@/lib/mailer";
...
function serializeMitarbeiter(person) {
  return {
    ...
    reminderLeadMinutes: person.reminderLeadMinutes,
    reminderLeadOptions: Array.isArray(person.reminderLeadOptions)
      ? person.reminderLeadOptions
      : person.reminderLeadMinutes
        ? [person.reminderLeadMinutes]
        : [],
    reminderConfigs: Array.isArray(person.reminderConfigs) ? person.reminderConfigs : null,
    ...
    emailColors: {
      ...
      ...(person.emailColors && typeof person.emailColors === "object" ? person.emailColors : {}),
    },
  };
}
...
function reminderLead(value, fallback) {
  const parsed = Number(value);
  return Number.isInteger(parsed) && parsed >= 0 && parsed <= 30 * 24 * 60
    ? parsed
    : fallback;
}

function reminderLeadOptions(value, fallback) {
...
export async function PATCH(request) {
  try {
    ...
    data.reminderLeadMinutes = reminderLead(
      body.reminderLeadMinutes,
      existing.reminderLeadMinutes
    );
    if (Array.isArray(body.reminderLeadOptions)) {
      const options = reminderLeadOptions(body.reminderLeadOptions, existing.reminderLeadOptions);
      data.reminderLeadOptions = options;
      data.reminderLeadMinutes = options[0] || 0;
    }
    if (Array.isArray(body.reminderConfigs)) {
      const configs = body.reminderConfigs
        .filter(
          (c) =>
            c &&
            typeof c === "object" &&
            ...
            Number(c.days) >= 0 &&
            Number(c.days) <= 30 &&
            Number.isInteger(Number(c.hour)) &&
            Number(c.hour) >= 0 &&
            Number(c.hour) <= 23 &&
            Number.isInteger(Number(c.minute)) &&
            Number(c.minute) >= 0 &&
            Number(c.minute) <= 59
        )
        .map((c) => ({ days: Number(c.days), hour: Number(c.hour), minute: Number(c.minute) }));
      data.reminderConfigs = configs;
    }
    ...
    if (body.emailColors && typeof body.emailColors === "object") {
      const validHex = (v) => typeof v === "string" && /^#[0-9a-fA-F]{6}$/.test(v);
      const col = body.emailColors;
      data.emailColors = {
        headerBg:   validHex(col.headerBg)   ? col.headerBg   : DEFAULT_EMAIL_COLORS.headerBg,
        footerBg:   validHex(col.footerBg)   ? col.footerBg   : DEFAULT_EMAIL_COLORS.footerBg,
        confirmBtn: validHex(col.confirmBtn) ? col.confirmBtn : DEFAULT_EMAIL_COLORS.confirmBtn,
        meetBtn:    validHex(col.meetBtn)    ? col.meetBtn    : DEFAULT_EMAIL_COLORS.meetBtn,
      };
    }
